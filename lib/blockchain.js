...
function getBlockNumber (txHash) {
  if (!_.startsWith(txHash, '0x') && !_.startsWith(txHash, '0X')) {
    txHash = `0x${txHash}`
  }
  ...
  return Promise.all([web3.eth.getTransaction(txHash), web3.eth.getTransactionReceipt(txHash)])
    .spread((transaction, receipt) => {
      if (transaction && receipt) {
        if (_.get(transaction, 'blockNumber') === _.get(receipt, 'blockNumber') && _.get(transaction, 'blockNumber') > 0 && _.get(receipt, 'blockNumber') > 0) {
          return Promise.resolve(_.get(transaction, 'blockNumber'))
        } else {
...
function sendTransaction (fromAccount, fromAccountPassword, toAccount, data, value = 1) {
  if (!_.startsWith(data, '0x') && !_.startsWith(data, '0X')) {
    data = `0x${data}`
  }
